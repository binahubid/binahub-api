const baseUrl = (process.env.PHASE10_API_URL || "http://localhost:3001").replace(/\/$/, "");
const failures = [];
...
const publicCatalog = await request("/api/catalog/modules?phase10=1", {
...
if (failures.length) {
  console.error(`\nPhase 10 smoke gate failed (${failures.length} check).`);
  process.exit(1);
}

console.log(`\nPhase 10 smoke gate passed against ${baseUrl}.`);
