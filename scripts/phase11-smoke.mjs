const baseUrl = (process.env.PHASE11_API_URL || "http://localhost:3001").replace(/\/$/, "");
const failures = [];
...
const publicCatalog = await request("/api/catalog/modules?phase11=1", {
...
if (failures.length) {
  console.error(`\nPhase 11 smoke gate failed (${failures.length} check).`);
  process.exit(1);
}

console.log(`\nPhase 11 smoke gate passed against ${baseUrl}.`);
