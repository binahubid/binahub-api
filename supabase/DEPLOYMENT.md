...
5. Terapkan `0015_ceo_revision_hardening.sql` sampai `0037_phase12_pilot_rehearsal_certification.sql`, masing-masing sebagai satu file penuh dan sesuai urutan. File `0034` menambahkan Human UAT & Pilot Gate, `0035` menambahkan controlled pilot release/runtime ceiling/kill switch, `0036` menambahkan Operational Assurance, incident response, dan go/no-go evidence, serta `0037` menambahkan production dry-run rehearsal dan final acceptance certification.
...
2. Terapkan migration `binahub-api/supabase/migrations/0005` sampai `0037` sebagai raw SQL secara leksikografis, bukan sebagai riwayat kedua `db push`.
